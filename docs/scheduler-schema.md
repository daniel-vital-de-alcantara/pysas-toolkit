...
PySAS 0.3.6 reads an Excel workbook and uses the worksheet named `Schedule` when it exists; otherwise it reads the first worksheet.
...
| `stop_program_on_error` | Boolean-like value | Reserved scheduler control field in 0.3.6; retained in task metadata but not currently used to alter execution independently. |
...
Before execution PySAS 0.3.6 checks key structural conditions including:
